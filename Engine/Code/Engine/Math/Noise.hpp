...
Vector2f GetPseudoRandomNoiseUnitVector2D( int positionX, int positionY );
...
//---------------------------------------------------------------------------
inline Vector2f GetPseudoRandomNoiseUnitVector2D( int xPosition, int yPosition )
{
	const float ONE_OVER_MAX_POSITIVE_INT = (1.f / 2147483648.f);
	const float SCALE_FACTOR = ONE_OVER_MAX_POSITIVE_INT * fTWO_PI;
	...
	return Vector2f( cos( pseudoRandomFloatZeroToTwoPi ), sin( pseudoRandomFloatZeroToTwoPi ) );
}
