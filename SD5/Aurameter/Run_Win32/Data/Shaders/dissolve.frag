...
uniform sampler2D uTexDissolve; //For dissolve% thresholding values.
...
//	finalColor = passthroughColor; 
	...
	float dissolveValue = texture( uTexDissolve, passUV0 ).r;

	if ( ( dissolveValue * uWrappingTimerDuration ) < uWrappingTimer )
		discard;
	
	if ( ( dissolveValue * uWrappingTimerDuration ) < uWrappingTimer+0.04 ) //Applies burn-y red color to fringes of dissolve.
		finalColor *= vec4(0.8, 0.2, 0.0, 1.0);
	//Else color will be normal.
