...
//Toon Shader: calc pixel luminance, apply Sobel edge detect filter to get magnitude; if mag > threshold use outline color else quantize pixel color.
void main()
{	
	...
	{
		outColor = vec4( 0., 0., 0., 1. ); //Black for outline for now.
	}
	else
	{
		diffuse.rgb *= uQuantizationLevel;
		diffuse.rgb += vec3( .5 ); //round
		ivec3 colorInt = ivec3( diffuse.rgb ); //truncate
		diffuse.rgb = vec3( colorInt ) / uQuantizationLevel; //"normalize" over total quantization level.
