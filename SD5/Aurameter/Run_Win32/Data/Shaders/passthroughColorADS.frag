...
//	vec4 diffuseTexColor = texture(uTexDiffuse, passUV0 + passUV1 + passUV2 + passUV3) + (.001f * passthroughColor);
...
//	vec3 emissiveColor = diffuseTexColor.rgb * emissiveStrength;
	...
	totalSurfaceLight = clamp( ( ambientColor + totalSurfaceLight ), vec3(0.0), vec3(1.0) );
	vec4 surfaceColor = passthroughColor * vec4( totalSurfaceLight, 1.0 ) + vec4( totalSpecularLight, 0.0 );
